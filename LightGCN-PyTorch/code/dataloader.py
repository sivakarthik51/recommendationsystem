...
class MovieLens(BasicDataset):
    ...
    def __init__(self, config=world.config, path="../data/movielens/"):
        ...
        train_df = pd.read_csv(train_file, usecols=['user', 'movie']) #, dtype={'user_id':np.int32, 'book_id':np.int32})
        test_df = pd.read_csv(test_file, usecols=['user', 'movie']) #, dtype={'user_id':np.int32, 'book_id':np.int32})
        
        self.trainItem = np.array(train_df['movie'])
        self.trainUser = np.array(train_df['user'])
        self.testItem = np.array(test_df['movie'])
        self.testUser = np.array(test_df['user'])
